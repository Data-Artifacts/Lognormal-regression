...
#  magyar nyelvű hónapnevek, hétnapja, stb:  locale.setlocale(locale.LC_TIME, "hungarian")
def fn_today(format='date'):
    ''' format:  'c10' ('c10.','.')   'c10-' ('-')   'c8'   date
    '''
    if format=='c10' or format=='c10.' or format=='.':
        ...
        return date.today()     # ilyenkor érdektelen az időzóna

def fn_now(format='datetime',tz=None):
    ''' Aktuális időpont
    format:  'datetime', 'unix',  'c10' (yyyy.MM.dd)   'c16' (yyyy.MM.dd hh:mm)   'c19' (yyyy.MM.dd hh:mm:ss)
             'to_path' (yyyyMMdd hhmmss)
        - megadható egyedi formátum-string is  (strftime)
    tz: None,  'Europe/Budapest'    ha nincs megadva, akkor "timezone naiv" datetime érték lesz az output
       FONTOS:  naív és aware datetime értékekre nem képezhető különbség  (mindkettőnek vagy naívnak vagy aware-nek kell lennie)
    '''
    if tz=='': tz=None
    elif type(tz)==str: tz=pytz.timezone(tz)
    else: tz=None
...
def datestr(datetime_or_float,format='c10'):    # gyorsírásos vagy standard python formázások;  float input is jó
    
    if type(datetime_or_float)==str: return datetime_or_float    # string input esetén nem csinál semmit
    elif isinstance(datetime_or_float,(float,int)): datetime=floatdate(datetime_or_float)
    else: datetime=datetime_or_float
...
def fn_dateadd(dt,nToAdd,unit='day'):
    if dt=='ma': dt=date.today()
    elif type(dt)==str: dt=Date(dt)
...
def fn_daydiff(dt1,dt2):
    if type(dt1)==str: dt1=Date(dt1)
    if type(dt2)==str: dt2=Date(dt2)
...
def fn_minutediff(dt1,dt2):
    # percben számított eltérés (lebegőpontos)
    return (dt1-dt2) / timedelta(minutes=1)



def fn_unixtimestampToStr(timestamp,dateformat='%Y.%m.%d %H:%M:%S',tz='Europe/Budapest'):
    # timezone:   azért kell, mert pl. a COREtxt szerveren nincs beállítva az időzóna
    #    none vagy '' esetén nincs tz beállítás
    # dateformat:  'datetime' esetén datetime lesz a return (nem string)
    if tz=='': tz=None
    elif tz: tz=pytz.timezone(tz)

    if dateformat=='datetime': return datetime.fromtimestamp(timestamp,tz)
    else: return datetime.fromtimestamp(timestamp,tz).strftime(dateformat)


def fn_unixtimestamp(dt):
    return dt.timestamp()


def fn_midnight(dt='ma',inday='end'):
    '''A megadott nap 23:59:59.999999 időpontja
    inday:  "end" a nap utolsó pilanata,   "start": a nap első pillanata
    '''
    if dt=='ma': dt=date.today()
    if inday=='start': return datetime.combine(dt, datetime.min.time())
    else: return datetime.combine(dt, datetime.max.time())
...
def fn_holiday(dt):
    '''
    Tömeges hívás esetén jobb csak egyszer bekérni a holidays objektumot (egyszerre akár több évre)
    '''
    holidays_=holidays.country_holidays('HU',years=dt.year)
    return dt in  holidays_





def fn_parsedate(strDate,format='.'):                  # lásd még:   Date, strdate    (pd.to_datetime algoritmus)
    ...
    if format=='c10.' or format=='.':
        ...
        return datetime.strptime(strDate,'%Y.%m.%d %H:%M')
    elif format=='c19':
        return datetime.strptime(strDate,'%Y.%m.%d %H:%M:%S')

def Date(strDate,format=None):          # pd.to_datetime
    '''
    Felismeri a legtöbb formátumot.
    format:  gyorsabb a parse, ha meg van adva
    '''
    return to_datetime(strDate,format=format)

strdate=Date
strdatefloat=datefloat                   # float to "yyyy.MM.dd",     datestr(), float input-tal
floatdatestr=datestr                     # "yyyy.MM.dd"  to  float    datefloat(), string input-tal
...
class Ttext2date:
    ...
    def sub_dátum_sorszám_törtrész(dátum,szám,törtrész):
        ...
        if szám==oszto:
            dtout=fn_dateadd(dt2,-Round(unit),'day')
            dtout2=dt2
        else:
            dtout=fn_dateadd(dt1,Round(unit*(szám-1)),'day')
            dtout2=fn_dateadd(dtout,Round(unit),'day')
        return (dtout,dtout2)
